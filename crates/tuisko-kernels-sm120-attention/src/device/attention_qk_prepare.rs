...
#[inline(always)]
unsafe fn store_bf16x8(destination: *mut u16, values: [f32; VALUES_PER_LANE]) {
    ...
    unsafe {
        ...
        *destination.add(2) = tcgen05::cvt_f32x2_bf16x2(values[4], values[5]);
        *destination.add(3) = tcgen05::cvt_f32x2_bf16x2(values[6], values[7]);
    }
}

#[inline(always)]
unsafe fn copy_bf16x8(destination: *mut u16, source: *const u16) {
    let destination = destination.cast::<u32>();
...
/// Prepares exact BF16 query/key rows and appends them to the paged cache.
///
/// # Safety
///
...
#[inline(always)]
#[allow(clippy::too_many_arguments)]
pub unsafe fn bf16_attention_qk_prepare<A: Arch, const TOKENS: usize>(
    qkv: *const u16,
    query_norm: *const u16,
    key_norm: *const u16,
    rope_cos: *const f32,
    ...
    let cache_element = A::HEAD_DIM
        * (page_offset + PAGE_SIZE * (kv_head + A::NUM_KV_HEADS * physical_page))
        + dimension;
    unsafe { store_bf16x8(key_pages.add(cache_element), prepared) };
    ...
    unsafe { copy_bf16x8(value_pages.add(cache_element), value_source) };
}
