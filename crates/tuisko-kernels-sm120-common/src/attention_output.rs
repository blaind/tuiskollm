//! Attention-gate reducers and output projection shared by the FP8, NVFP4 and MTP families.
...
/// Reduces the attention gate and writes it back as BF16.
///
/// # Safety
///
/// The planes must address one complete row per launched block.
#[inline(always)]
pub unsafe fn attention_gate_bf16<A: Arch>(
    attention: *mut f32,
    qkv: *const u16,
    activation: *mut u16,
) {
...
/// Reduces the attention gate and quantizes it to E4M3 plus a scale.
///
/// # Safety
///
/// The planes must address one complete row per launched block.
#[inline(always)]
pub unsafe fn attention_gate_quantize<A: Arch>(
    attention: *mut f32,
    qkv: *const u16,
    codes: *mut u16,
    scales: *mut f32,
...
/// # Safety
///
/// The planes must address one complete tile per launched block.
#[inline(always)]
pub unsafe fn attention_output_projection_mma<
    A: Arch,
    const TOKENS: usize,
