...
use crate::{
    ...
    PagedKvTableUpdate, QWEN36_LONG_CONTEXT_PHYSICAL_PAGES, QWEN36_MAX_CONTEXT_TOKENS,
...
pub struct Qwen36LongContextKvProgram {
    ...
    layout: Qwen36LongContextKvLayout,
}

impl Qwen36LongContextKvProgram {
    /// Allocates the complete 262,144-position page pool.
    pub fn new(context: &Arc<CudaContext>) -> EngineResult<Self> {
        Self::new_for_pages(context, QWEN36_LONG_CONTEXT_PHYSICAL_PAGES)
    }

    fn new_for_pages(context: &Arc<CudaContext>, physical_pages: usize) -> EngineResult<Self> {
        let layout = Qwen36LongContextKvLayout::build_for_pages(physical_pages)?;
    ...
    pub fn qualification_cache_page(
        ...
        if physical_page >= self.layout.physical_pages() {
            return Err(EngineError::route(format!(
                "Qwen3.6 physical KV page {physical_page} is outside 0..{}",
                self.layout.physical_pages()
            )));
        }
        let mut key = Vec::new();
        ...
        for layer in self.layout.layers() {
            ...
            let start = physical_page
                ...
                .ok_or_else(|| EngineError::layout("Qwen3.6 KV page offset overflows"))?;
            key.extend(
                self.arena
    ...
    fn upload_entries(
        ...
        let destination = slot
            .checked_mul(QWEN36_LONG_CONTEXT_PHYSICAL_PAGES)
            .and_then(|begin| begin.checked_add(first_entry))
            .ok_or_else(|| EngineError::layout("Qwen3.6 KV table upload offset overflows"))?;
        let end = first_entry
            .checked_add(entry_count)
            .ok_or_else(|| EngineError::layout("Qwen3.6 KV table upload range overflows"))?;
...
#[cfg(test)]
mod tests {
    use super::{QWEN36_LONG_CONTEXT_PHYSICAL_PAGES, QWEN36_MAX_CONTEXT_TOKENS};
