...
/// Concrete Qwen3.6 scheduler for up to eight compact decode rows.
pub struct Qwen36ResidentBatchGenerator {
    frontend: TextFrontend,
    program: Qwen36ResidentModelProgram,
    stream: Arc<CudaStream>,
    logits: PinnedHostBuffer<u16>,
    sessions: [Option<Qwen36BatchSession>; MAX_BATCH],
    active_slots: [usize; MAX_BATCH],
    active: usize,
    next_request_id: u64,
}

struct Qwen36BatchSession {
...
impl Qwen36ResidentBatchGenerator {
    /// Opens the exact Qwen3.6 compact scheduler on device zero.
    ...
    pub fn from_snapshot(
        ...
        let logit_values = Qwen36Moe35B::VOCAB
            .checked_mul(LOGIT_BANK_ROWS)
            .ok_or_else(|| EngineError::layout("Qwen3.6 compact logit banks overflow"))?;
    ...
    pub fn admit(
        ...
        self.next_request_id = self
            .next_request_id
            .checked_add(1)
            .ok_or_else(|| EngineError::generation("Qwen3.6 request identity overflows"))?;
        ...
        let slot = first_free_slot(occupied)
            .ok_or_else(|| EngineError::route("all eight Qwen3.6 generation slots are active"))?;
        ...
        let native_prefill_tokens = prime_qwen36_prompt(
            &mut self.program,
            &self.stream,
        ...
        self.sessions[slot] = Some(Qwen36BatchSession {
            request_id,
            control,
            pending_token: None,
            next_position,
    ...
    pub fn step(&mut self) -> EngineResult<ResidentBatchEvents> {
        if self.active == 0 {
            return Err(EngineError::generation(
                "cannot step an empty Qwen3.6 generation scheduler",
            ));
        }
        self.replay_pending()?;
        ...
        for (index, event) in events[..active].iter_mut().enumerate() {
            ...
            let step = {
                let session = self.sessions[slot].as_mut().ok_or_else(|| {
                    EngineError::generation("active Qwen3.6 slot has no generation session")
                })?;
            ...
            let request_id = self.sessions[slot]
                .as_ref()
                .expect("active Qwen3.6 session survived sampling")
                .request_id;
            let completed = if step.finish_reason.is_some() {
                let session = self.sessions[slot]
                    .take()
                    .expect("terminal Qwen3.6 session exists");
                self.program.recycle_kv_slot(&self.stream, slot)?;
    ...
    pub fn cancel(&mut self, request_id: ResidentRequestId) -> EngineResult<ResidentCancellation> {
        let index = self.active_slots[..self.active]
            ...
            .ok_or_else(|| EngineError::generation("Qwen3.6 cancellation request is not active"))?;
        let slot = self.active_slots[index];
        let session = self.sessions[slot]
            .take()
            .expect("cancelled Qwen3.6 slot owns a session");
    ...
    pub fn active_request_ids(&self) -> impl Iterator<Item = ResidentRequestId> + '_ {
        self.active_slots[..self.active].iter().map(|&slot| {
            self.sessions[slot]
                .as_ref()
                .expect("active Qwen3.6 slot owns a session")
                .request_id
        })
    }

    /// Exact bytes across all resident Qwen3.6 arenas.
    pub const fn arena_bytes(&self) -> usize {
        self.program.layout().arena_bytes()
    }

    /// Source-backed Qwen3.6 weights resident on the device.
    ...
    fn replay_pending(&mut self) -> EngineResult<()> {
        ...
        for &slot in &self.active_slots[..self.active] {
            let session = self.sessions[slot].as_ref().ok_or_else(|| {
                EngineError::generation("active Qwen3.6 slot has no pending session")
            })?;
        ...
        for (row, &slot) in slots[..pending].iter().enumerate() {
            self.logits
                .copy_within(compact_download_row(row), slot * Qwen36Moe35B::VOCAB);
            let session = self.sessions[slot]
                .as_mut()
                .expect("pending Qwen3.6 slot owns a session");
...
fn compact_download_row(row: usize) -> Range<usize> {
    let begin = (MAX_BATCH + row) * Qwen36Moe35B::VOCAB;
    begin..begin + Qwen36Moe35B::VOCAB
}
