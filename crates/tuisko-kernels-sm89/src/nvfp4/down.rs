...
// produce 16 rows; 5,120 / 16 = 320 CTAs provide 2.5 waves on a 128-SM RTX 4090.
// Staging each 512-value phase once removes 15 duplicate activation reads per
// CTA while preserving both dot-product accumulation orders.
const WARPS: usize = 8;
...
/// Prepared A16 routes consuming the exact source NVFP4 down owner on SM89.
pub struct Nvfp4DownOp<A: Sm89Arch = Qwen38_27B> {
    module: kernels::LoadedModule,
...
impl<A: Sm89Arch> Nvfp4DownOp<A> {
    ...
    pub fn new(context: &Arc<CudaContext>) -> GpuResult<Self> {
        ...
        let module = unsafe { kernels::load(context) }.map_err(|source| {
            GpuError::module("loading the SM89 NVFP4 down projection module", source)
        })?;
