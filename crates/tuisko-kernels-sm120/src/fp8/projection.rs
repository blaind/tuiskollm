//! Source-native FP8 full-attention QKV projection.

use crate::device::fp8_projection::{qkv_projection, quantize_activation};
...
const QKV_WARPS: usize = 8;
const QKV_THREADS: u32 = (QKV_WARPS * 32) as u32;
...
mod kernels {
    ...
    pub fn fp8_qkv<A: Arch, const TOKENS: usize>(
        ...
        unsafe {
            qkv_projection::<A, TOKENS, QKV_WARPS>(
                activation_codes,
                activation_scales,
                weight_codes,
                weight_scales,
...
impl<A: Arch, const TOKENS: usize> PreparedQkvRoute<A, TOKENS> {
    fn prepare(module: &kernels::LoadedModule) -> GpuResult<Self> {
        let quantize_blocks = u32::try_from(TOKENS)
            .map_err(|_| GpuError::invalid_launch("FP8 QKV batch exceeds CUDA grid width"))?;
        let row_pairs_per_block = 2 * QKV_WARPS;
        ...
        let quantize = module
            .prepare_quantize_activation_e4m3(LaunchConfig1D::new(
                quantize_blocks,
                QUANTIZE_THREADS,
                0,
            ))
        ...
        let projection = module
            .prepare_fp8_qkv::<A, TOKENS>(LaunchConfig1D::new(projection_blocks, QKV_THREADS, 0))
...
/// PTX symbols retained for activation quantization and every exact QKV batch.
pub(crate) fn fp8_qkv_ptx_names() -> [&'static str; 9] {
    [
        "quantize_activation_e4m3",
...
impl FullAttentionQkvOp {
    /// Loads the embedded SM120 module and prepares every exact decode route.
    ...
    /// Dynamically quantizes `batch` BF16 rows and projects fused Q/K/V output.
    ///
    /// # Safety
    ///
    ...
    /// 14,336 BF16 scales; and `output` covers `batch * 14_336` BF16 values.
    ...
    pub unsafe fn launch(
        ...
        match batch {
            ...
            _ => Err(GpuError::invalid_launch(format!(
                "FP8 QKV batch {batch} is outside the exact range 1..={MAX_BATCH}"
            ))),
        }
    }
}
...
#[cfg(test)]
mod tests {
    use super::{MAX_BATCH, QKV_THREADS, QKV_WARPS, QUANTIZE_THREADS, fp8_qkv_ptx_names};
    ...
    fn exact_geometry_matches_the_retained_decode_schedule() {
        ...
        assert_eq!(QKV_THREADS, 256);
        assert_eq!(QKV_WARPS, 8);
        assert_eq!(Qwen38_27B::ATTENTION_QKV_ROWS % (2 * QKV_WARPS), 0);
