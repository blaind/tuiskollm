//! Exact Qwen3.5 FP32 GDN recurrence and gated normalization.
...
mod kernels {
    ...
    /// Advances mapped Qwen3.5 state and emits gated normalized values.
    #[allow(clippy::too_many_arguments)]
    #[kernel]
    #[launch_bounds(512, 2)]
    #[launch_contract(
    ...
    pub fn qwen35_gdn_recurrence_exact<const TOKENS: usize>(
        ...
        if tid < HEAD_DIM {
            unsafe {
                *core::ptr::addr_of_mut!(QUERY).cast::<f32>().add(tid) =
                    load_bf16(query_row.add(tid));
                *core::ptr::addr_of_mut!(KEY).cast::<f32>().add(tid) = load_bf16(key_row.add(tid));
            }
        }
        thread::sync_threads();

        let query = core::ptr::addr_of_mut!(QUERY).cast::<f32>();
        let key = core::ptr::addr_of_mut!(KEY).cast::<f32>();
        let recurrent_output = core::ptr::addr_of_mut!(RECURRENT_OUTPUT).cast::<f32>();
...
/// Prepared recurrent-state routes for every exact Qwen3.5 decode batch.
...
impl Qwen35GdnRecurrenceOp {
    ...
    /// # Safety
    ///
    /// `qkv` and `projected` cover BF16 `[batch, 8_192]` and `[batch,
    /// 12_288]`. Controls cover FP32 `[batch, 32]`; `norm_weight` covers 128
    /// BF16 values. Every state row is within `[rows, 32, 128, 128]`, and
    /// `output` covers BF16 `[batch, 4_096]`. Allocations are aligned,
    /// disjoint, live through completion, and belong to the stream's context.
    #[allow(clippy::too_many_arguments)]
