//! Private T=1024 TMA route for the dense-FP8 GDN input projection.
...
const _: () = assert!(Qwen38_27B::GDN_INPUT_ROWS.is_multiple_of(BLOCK_N));
...
mod kernels {
    ...
    /// Applies the retained three-stage GDN input route at exactly T=1024.
    #[kernel]
    #[launch_bounds(288, 2)]
    #[launch_contract(
        domain = 1,
    ...
    pub fn fp8_gdn_input_tma_t1024(
        ...
        // this exact 5,120-wide projection into every 64-row GDN output tile.
        ...
        let output_tiles = Qwen38_27B::GDN_INPUT_ROWS / BLOCK_N;
        ...
        while task < output_vectors {
            ...
            unsafe {
                ...
                *output
                    .add(
                        (token_begin + token) * Qwen38_27B::GDN_INPUT_ROWS
                            + row_begin
                            + row_vector * 8,
                    )
                    .cast::<[u32; 4]>() = values;
...
/// Address-bound tensor maps for the exact dense-FP8 GDN input macro route.
pub struct DenseFp8GdnInputTmaMaps {
...
impl DenseFp8GdnInputTmaMaps {
    ...
    /// `weight_codes` covers source-native `[GDN_INPUT_ROWS, 5120]` E4M3
    ...
    pub unsafe fn new(
        ...
        let weight = create_map(
            ...
            Qwen38_27B::HIDDEN as u64,
            Qwen38_27B::GDN_INPUT_ROWS as u64,
            Qwen38_27B::HIDDEN as u64,
            BLOCK_K as u32,
            BLOCK_N as u32,
        )?;
    ...
    pub fn copy_to_host(&self, stream: &CudaStream) -> GpuResult<[Vec<u64>; 2]> {
            ...
            || !Arc::ptr_eq(stream.context(), self.weight_code_map.context())
        {
            return Err(GpuError::invalid_launch(
                "dense-FP8 GDN input tensor maps belong to another CUDA context",
            ));
        }
        Ok([
...
impl DenseFp8GdnInputTmaRoute {
    pub(super) fn new(context: &Arc<CudaContext>) -> GpuResult<Self> {
        ...
        let module = unsafe { kernels::load(context) }
            .map_err(|source| GpuError::module("loading dense-FP8 GDN input TMA module", source))?;
        let blocks = ((TOKENS / BLOCK_M) * (Qwen38_27B::GDN_INPUT_ROWS / BLOCK_N)) as u32;
        let prepared = module
...
fn create_map(
    ...
    if result != cuda_sys::cudaError_enum_CUDA_SUCCESS {
        return Err(GpuError::invalid_launch(format!(
            "encoding dense-FP8 GDN input tensor map failed: {result:?}"
        )));
    }
...
mod tests {
    ...
    #[test]
    fn exact_tma_geometry_preserves_the_gdn_input_route() {
