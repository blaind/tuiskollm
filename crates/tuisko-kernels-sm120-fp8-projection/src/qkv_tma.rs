//! Private T=1024 TMA route for the dense-FP8 QKV projection.
...
const _: () = assert!(Qwen38_27B::HIDDEN.is_multiple_of(BLOCK_K));
const _: () = assert!(Qwen38_27B::ATTENTION_QKV_ROWS.is_multiple_of(BLOCK_N));
...
mod kernels {
    ...
    /// Applies the retained three-stage QKV route at exactly T=1024.
    #[kernel]
    #[launch_bounds(288, 2)]
    #[launch_contract(
        domain = 1,
    ...
    pub fn fp8_qkv_tma_t1024(
        ...
        // warps, identical to the down macro tile; the 80-tile K walk covers
        // this exact 5,120-wide projection into every 64-row QKV output tile.
        ...
        let output_tiles = Qwen38_27B::ATTENTION_QKV_ROWS / BLOCK_N;
        ...
        while task < output_vectors {
            ...
            unsafe {
                ...
                *output
                    .add(
                        (token_begin + token) * Qwen38_27B::ATTENTION_QKV_ROWS
                            + row_begin
                            + row_vector * 8,
                    )
                    .cast::<[u32; 4]>() = values;
...
/// Address-bound tensor maps for the exact dense-FP8 QKV macro route.
pub struct DenseFp8QkvTmaMaps {
...
impl DenseFp8QkvTmaMaps {
    ...
    /// # Safety
    ///
    /// `activation_codes` covers `[1024, 5120]` E4M3 bytes and
    /// `weight_codes` covers source-native `[ATTENTION_QKV_ROWS, 5120]` E4M3
    ...
    pub unsafe fn new(
        ...
        let weight = create_map(
            ...
            Qwen38_27B::HIDDEN as u64,
            Qwen38_27B::ATTENTION_QKV_ROWS as u64,
            Qwen38_27B::HIDDEN as u64,
            BLOCK_K as u32,
            BLOCK_N as u32,
        )?;
    ...
    pub fn copy_to_host(&self, stream: &CudaStream) -> GpuResult<[Vec<u64>; 2]> {
            ...
            || !Arc::ptr_eq(stream.context(), self.weight_code_map.context())
        {
            return Err(GpuError::invalid_launch(
                "dense-FP8 QKV tensor maps belong to another CUDA context",
            ));
        }
        Ok([
...
pub(super) struct DenseFp8QkvTmaRoute {
    module: kernels::LoadedModule,
    prepared: PreparedLaunch<kernels::__fp8_qkv_tma_t1024_CudaKernel>,
    ...
    prepared_t128: PreparedLaunch<kernels::__fp8_qkv_tma_t1024_CudaKernel>,
}

impl DenseFp8QkvTmaRoute {
    pub(super) fn new(context: &Arc<CudaContext>) -> GpuResult<Self> {
        ...
        let module = unsafe { kernels::load(context) }
            .map_err(|source| GpuError::module("loading dense-FP8 QKV TMA module", source))?;
        let blocks = ((TOKENS / BLOCK_M) * (Qwen38_27B::ATTENTION_QKV_ROWS / BLOCK_N)) as u32;
        let prepared = module
            .prepare_fp8_qkv_tma_t1024(LaunchConfig1D::new(
                blocks,
                THREADS as u32,
                SHARED_BYTES as u32,
            ))
            .map_err(|source| GpuError::launch("preparing dense-FP8 QKV T=1024", source))?;
        let t128_blocks = ((128 / BLOCK_M) * (Qwen38_27B::ATTENTION_QKV_ROWS / BLOCK_N)) as u32;
        let prepared_t128 = module
            ...
            .map_err(|source| GpuError::launch("preparing dense-FP8 QKV T=128", source))?;
    ...
    unsafe fn launch_prepared(
        ...
        self.module
            ...
            .map_err(|source| GpuError::launch("launching dense-FP8 QKV TMA", source))
    }
}
...
fn create_map(
    ...
    if result != cuda_sys::cudaError_enum_CUDA_SUCCESS {
        return Err(GpuError::invalid_launch(format!(
            "encoding dense-FP8 QKV tensor map failed: {result:?}"
        )));
    }
...
mod tests {
    use super::{
        BLOCK_K, BLOCK_M, BLOCK_N, CONSUMER_WARPS, K_TILES, PRODUCER_THREADS, SHARED_BYTES,
        THREADS, TOKENS,
    };
    use tuisko_model::{Arch, Qwen38_27B};

    #[test]
    fn exact_tma_geometry_preserves_the_qkv_route() {
        ...
        assert_eq!(K_TILES, Qwen38_27B::HIDDEN / 64);
    }
}
