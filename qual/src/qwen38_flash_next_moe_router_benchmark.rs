//! Paired timings for every exact Qwen3.8-Flash-Next MoE router route.
...
use crate::qwen38_flash_next_moe_router::{
...
struct Session {
    ...
    _op: Qwen38FlashNextMoeRouterOp,
    arena: DeviceArena,
    regions: Regions,
...
impl Session {
    fn new(repeated_operations: u64) -> Result<Self, DeviceBenchmarkError> {
        ...
        let op = Qwen38FlashNextMoeRouterOp::new(&context)?;
    ...
    fn cases(&self, repeated_operations: u64) -> Vec<ExactDeviceCase<'_>> {
        self.routes
            ...
            .map(|route| {
                ...
                ExactDeviceCase::new(
                    "qwen38_flash_next/moe_router/bf16_top10",
                    shape,
                    workload,
...
/// Measures every exact Qwen3.8-Flash-Next BF16 router and top-ten selection route.
pub fn benchmark_qwen38_flash_next_moe_router(
    ...
    memory.register_owned(
        "qwen38_flash_next/moe_router/weights",
        BenchmarkMemoryKind::Weights,
        weight_bytes,
        "source BF16 [512,2560] router matrix",
    )?;
    memory.register_owned(
        "qwen38_flash_next/moe_router/address_stable_workspace",
        BenchmarkMemoryKind::Workspace,
        session.arena.byte_len() - weight_bytes - padding_bytes,
        "max_rows=1024 logits, top-ten indices, and normalized weights",
    )?;
    memory.register_owned(
        "qwen38_flash_next/moe_router/alignment_padding",
    ...
    finish_report(
        BenchmarkReportSpec {
            suite: "bench-qwen38-flash-next-moe-router",
...
mod tests {
    use super::*;
    use crate::qwen38_flash_next_moe_router::MAX_ROWS;
    ...
    fn accounting_covers_router_projection_and_selection() {
        ...
        // Every byte the arena holds is a payload byte the accounting names.
