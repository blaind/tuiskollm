...
fn launch(
    ...
    // SAFETY: every pointer names its complete, aligned maximum-row region.
    unsafe {
        op.launch(
            stream,
            rows,
...
pub fn benchmark_nvfp4_swiglu(
    ...
    memory.register_owned(
        ...
        "max_rows=1024 activation quantization and fused gate/up output seams",
    )?;
    memory.register_owned(
...
mod tests {
    use super::{
        CODE_BYTES_PER_ROW, EXACT_ROUTES, GATE_UP_ROWS, GROUPS_PER_ROW, HIDDEN, MAX_ROWS,
        OUTPUT_ROWS, layout, logical_bytes,
    };
