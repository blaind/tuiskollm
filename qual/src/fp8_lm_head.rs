//! Numerical and graph qualification for exact FP8 LM-head projection batches.
...
/// Failure of the exact FP8 LM-head qualification gate.
#[derive(Debug, thiserror::Error)]
pub enum Fp8LmHeadQualificationError {
    /// GPU ownership, launch, or driver failure.
    #[error(transparent)]
    Gpu(#[from] GpuError),

    /// Device behavior disagreed with the independent contract.
    #[error("FP8 LM-head qualification failed: {0}")]
    Mismatch(String),
}

/// Observable counts and worst error from every exact LM-head batch.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct Fp8LmHeadQualification {
    ...
    /// Full-vocabulary BF16 logits compared with the represented-value oracle.
...
/// Qualifies eager and captured full-vocabulary LM-head projection at exact `B=1..=8`.
pub fn qualify_fp8_lm_head() -> Result<Fp8LmHeadQualification, Fp8LmHeadQualificationError> {
    ...
    let token_oracles = input
        ...
        .map_err(Fp8LmHeadQualificationError::Mismatch)?;
    ...
    let mut report = Fp8LmHeadQualification {
        activation_codes: 0,
        activation_scales: 0,
        output_values: 0,
...
fn verify_eager(
    ...
    observed: &Observed,
    report: &mut Fp8LmHeadQualification,
) -> Result<(), Fp8LmHeadQualificationError> {
    let hidden = Qwen38_27B::HIDDEN;
    let output_rows = Qwen38_27B::VOCAB;
...
fn verify_replay(
    ...
    report.graph_replay_values += batch * (Qwen38_27B::HIDDEN + 1 + Qwen38_27B::VOCAB);
    report.inactive_values += inactive_values(batch);

    Ok(())
}

fn verify_inactive(batch: usize, observed: &Observed) -> Result<(), Fp8LmHeadQualificationError> {
    ...
    let output_begin = batch * Qwen38_27B::VOCAB;
...
fn inactive_values(batch: usize) -> usize {
    (MAX_BATCH - batch) * (Qwen38_27B::HIDDEN + 1 + Qwen38_27B::VOCAB)
}

#[cfg(test)]
mod tests {
    use super::{Fp8LmHeadQualificationError, MAX_BATCH, Qwen38_27B, qualify_fp8_lm_head};
    ...
    -> Result<(), Fp8LmHeadQualificationError> {
        ...
        assert_eq!(report.output_values, active_rows * Qwen38_27B::VOCAB);
