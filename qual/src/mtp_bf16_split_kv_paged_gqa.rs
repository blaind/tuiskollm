//! Numerical and graph qualification for partitioned MTP BF16 split-KV paged GQA.

use crate::fp8_projection_oracle::{BYTE_SENTINEL, F32_SENTINEL_BITS, f32_to_bf16};
...
use tuisko_kernels_sm120::{
    ...
    LONG_CONTEXT_GQA_PARTITION_SIZE, MtpBf16SplitKvPagedGqaOp,
};
...
const KEY_VALUES: [f32; 9] = [0.0, 0.25, 0.5, 1.0, -0.25, -0.5, -1.0, 0.125, -0.125];
const VALUE_VALUES: [f32; 9] = [1.0, -1.0, 0.5, -0.5, 0.25, -0.25, 0.125, -0.125, 0.0];

/// Failure of the partitioned MTP BF16 split-KV paged GQA gate.
#[derive(Debug, thiserror::Error)]
pub enum MtpBf16SplitKvPagedGqaQualificationError {
    ...
    #[error("MTP BF16 split-KV paged GQA qualification failed: {0}")]
    Mismatch(String),
}

/// Observable counts and worst errors across every exact route.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct MtpBf16SplitKvPagedGqaQualification {
...
pub fn qualify_mtp_bf16_split_kv_paged_gqa()
-> Result<MtpBf16SplitKvPagedGqaQualification, MtpBf16SplitKvPagedGqaQualificationError> {
    let _preflight = device_benchmark::preflight()?;
    let context = CudaContext::new(0).map_err(GpuError::from)?;
    let capability = context.compute_capability().map_err(GpuError::from)?;
    if capability != (12, 0) {
        return Err(MtpBf16SplitKvPagedGqaQualificationError::Mismatch(format!(
    ...
    let op = MtpBf16SplitKvPagedGqaOp::new(&context)?;
    let stable_addresses = addresses(&arena, regions)?;
    let mut report = MtpBf16SplitKvPagedGqaQualification {
        output_values: 0,
...
fn fixture() -> Fixture {
    ...
    let plane_elements =
        PHYSICAL_PAGES * Qwen38_27B::NUM_KV_HEADS * ATTENTION_PAGE_SIZE * Qwen38_27B::HEAD_DIM;
    let mut key_pages = vec![0u16; plane_elements];
    let mut value_pages = vec![0u16; plane_elements];
    for physical in 0..PHYSICAL_PAGES {
        for head in 0..Qwen38_27B::NUM_KV_HEADS {
            for position in 0..ATTENTION_PAGE_SIZE {
                for class in 0..8 {
                    let key = f32_to_bf16(
                        KEY_VALUES[(physical * 5 + head * 3 + position + class * 2) % 9],
                    );
                    let value = f32_to_bf16(
                        VALUE_VALUES[(physical * 7 + head + position * 2 + class * 3) % 9],
                    );
...
fn oracle(fixture: &Fixture) -> Result<Oracle, MtpBf16SplitKvPagedGqaQualificationError> {
                ...
                for position in first..end {
                    ...
                    while class < 8 {
                        ...
                        score += 32.0
                            ...
                            * f64::from(f32::from_bits(u32::from(key) << 16));
                        class += 1;
                    }
                    score *= 0.0625;
                    ...
                    while class < 8 {
                        let value = fixture.value_pages
                            [cache_offset(physical, kv_head, page_position, class)];
                        numerator[class] +=
                            weight * f64::from(f32::from_bits(u32::from(value) << 16));
                        class += 1;
                    }
                }
...
fn verify_oracle(
    ...
    oracle: &Oracle,
    observed: &Observed,
    report: &mut MtpBf16SplitKvPagedGqaQualification,
) -> Result<(), MtpBf16SplitKvPagedGqaQualificationError> {
    verify_scalar_plane(
        batch,
        "partial maximum",
...
mod tests {
    use super::{
        ...
        numerator_elements, qualify_mtp_bf16_split_kv_paged_gqa, scalar_elements,
    };
    ...
    fn mtp_bf16_paged_gqa_suite_split_kv_exact_batches_match_independent_oracles_and_graph_replay()
    -> Result<(), super::MtpBf16SplitKvPagedGqaQualificationError> {
        let report = qualify_mtp_bf16_split_kv_paged_gqa()?;
        ...
        assert_eq!(report.arena_bytes, 1_072_158_720);
