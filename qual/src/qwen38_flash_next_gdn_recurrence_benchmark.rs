//! Paired timings for exact Qwen3.8-Flash-Next FP32 GDN recurrence graph routes.
...
struct Session {
    ...
    _op: Qwen38FlashNextGdnRecurrenceOp,
    arena: DeviceArena,
    regions: Regions,
    ...
    fn cases(&self) -> Vec<ExactDeviceCase<'_>> {
        self.routes
            ...
            .map(|route| {
                ...
                ExactDeviceCase::new(
                    "qwen38_flash_next/gdn_recurrence/state_sigmoid_gate_norm",
                    shape,
                    workload,
...
fn load_fixture(arena: &DeviceArena, stream: &CudaStream, regions: Regions) -> GpuResult<Vec<f32>> {
    let qkv = (0..MAX_ROWS * Qwen38FlashNext::GDN_QKV_ROWS)
        .map(|index| f32_to_bf16(VALUES[(3 * index + index / Qwen38FlashNext::GDN_QKV_ROWS) & 7]))
        .collect::<Vec<_>>();
...
fn logical_bytes(rows: usize, prefill: bool) -> usize {
    let per_token = 2 * Qwen38FlashNext::GDN_QKV_ROWS
        + 2 * VALUE_WIDTH
    ...
    let recurrent_plane = usize::from(prefill) * 2 * rows * VALUE_WIDTH * size_of::<f32>();

    HEAD_DIM * size_of::<u16>() + rows * per_token + recurrent_plane
}

/// Measures every exact Qwen3.8-Flash-Next recurrence route with paired timings.
pub fn benchmark_qwen38_flash_next_gdn_recurrence(
    ...
    memory.register_owned(
        "qwen38_flash_next_gdn_recurrence/norm_weight",
        BenchmarkMemoryKind::Weights,
        weight_bytes,
        "one BF16 head-width norm vector",
    )?;
    memory.register_owned(
        "qwen38_flash_next_gdn_recurrence/address_stable_state",
        BenchmarkMemoryKind::Other,
        state_bytes,
        "eight FP32 recurrent state rows",
    )?;
    memory.register_owned(
        "qwen38_flash_next_gdn_recurrence/address_stable_workspace",
        BenchmarkMemoryKind::Workspace,
        workspace_bytes,
        "max_rows=1024 inputs, controls, mapping, recurrent plane, and output",
    )?;
    memory.register_owned(
        "qwen38_flash_next_gdn_recurrence/alignment_padding",
        BenchmarkMemoryKind::Other,
        padding_bytes,
    ...
    finish_report(
        BenchmarkReportSpec {
            suite: "bench-qwen38-flash-next-gdn-recurrence",
...
mod tests {
    use super::{
        CAUSAL_ROUTES, EXACT_ROUTES, HEAD_DIM, MAX_BATCH, MAX_ROWS, STATE_PER_ROW, VALUE_HEADS,
        VALUE_WIDTH, benchmark_routes, layout, logical_bytes,
    };
    ...
    fn byte_accounting_includes_the_complete_state_transition() {
        let per_token = 2 * Qwen38FlashNext::GDN_QKV_ROWS
            ...
            + 2 * STATE_PER_ROW * 4
            + VALUE_WIDTH * 2;
        let recurrent_plane_per_token = 2 * VALUE_WIDTH * 4;
        assert_eq!(logical_bytes(1, false), HEAD_DIM * 2 + per_token);
        assert_eq!(
        ...
        assert_eq!(
            ...
            HEAD_DIM * 2 + per_token + recurrent_plane_per_token
        );
        assert_eq!(
            logical_bytes(MAX_ROWS, true),
            HEAD_DIM * 2 + MAX_ROWS * (per_token + recurrent_plane_per_token)
        );
    }
    ...
    fn benchmark_route_inventory_and_owner_accounting_are_exact() {
        ...
        assert_eq!(regions.recurrent_plane.byte_len(), 25_165_824);
