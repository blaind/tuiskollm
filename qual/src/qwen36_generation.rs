//! Source-backed integration gate for the Qwen3.6 frontend and resident text graph.

use crate::{DeviceBenchmarkError, device_benchmark};
use std::path::Path;
use std::sync::Arc;
use tuisko_engine::{
    ChatGenerationRequest, EngineError, FinishReason, GeneratedText, Qwen36ResidentBatchGenerator,
    Qwen36ResidentTextGenerator, ResidentBatchEvents, ResidentRequestId, SamplingOptions,
};
...
pub enum Qwen36GenerationQualificationError {
    ...
    #[error("Qwen3.6 generation qualification failed: {0}")]
    Mismatch(String),
}

/// Frontend, ownership, and streaming boundaries checked by the integration gate.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct Qwen36GenerationQualification {
...
/// Exact compact routes and lifecycle seams checked for Qwen3.6 serving.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct Qwen36CompactGenerationQualification {
...
/// Qualifies the exact single-slot Qwen3.6 frontend-to-device generation path.
pub fn qualify_qwen36_generation(
    root: &Path,
) -> Result<Qwen36GenerationQualification, Qwen36GenerationQualificationError> {
    let _preflight = device_benchmark::preflight()?;
    let snapshot = Arc::new(CheckpointSnapshot::<Qwen36Moe35B>::open(root)?);
    let oracle_frontend = TextFrontend::open_qwen36(snapshot.as_ref())?;
    ...
    let mut generator = Qwen36ResidentTextGenerator::from_snapshot(&context, snapshot)?;
    ...
    Ok(Qwen36GenerationQualification {
        prompt_cases: 2,
...
/// Qualifies compact Qwen3.6 decode, admission, cancellation, and slot reuse.
pub fn qualify_qwen36_compact_generation(
    root: &Path,
) -> Result<Qwen36CompactGenerationQualification, Qwen36GenerationQualificationError> {
    let _preflight = device_benchmark::preflight()?;
    let snapshot = Arc::new(CheckpointSnapshot::<Qwen36Moe35B>::open(root)?);
    ...
    let mut generator = Qwen36ResidentBatchGenerator::from_snapshot(&context, snapshot)?;
    verify_compact_owner(&generator)?;
    let stable_addresses = generator.qualification_addresses();

    let request = greedy_request("Hello", 2);
    let alternate = greedy_request("Name one color.", 2);
    let exact_prefill = exact_prompt_request(&frontend, 128)?;
    ...
    if generator.qualification_addresses() != stable_addresses {
        return Err(Qwen36GenerationQualificationError::Mismatch(
            "Qwen3.6 compact owner addresses changed".into(),
        ));
    }
    device_benchmark::require_current_process_exclusive()?;

    Ok(Qwen36CompactGenerationQualification {
        route_batches: 8,
        requests: 4,
        recycled_slot,
        concurrent_prefill_tokens: 128,
        cancellations: 1,
        arena_bytes: generator.arena_bytes(),
        host_stager_bytes: generator.host_stager_bytes(),
        stable_addresses: stable_addresses.len(),
...
fn exact_prompt_request(
    ...
    target_tokens: usize,
) -> Result<ChatGenerationRequest, Qwen36GenerationQualificationError> {
...
) -> Result<GeneratedText, Qwen36GenerationQualificationError> {
...
fn verify_hole_reuse(
...
) -> Result<usize, Qwen36GenerationQualificationError> {
    ...
    {
        return Err(Qwen36GenerationQualificationError::Mismatch(
            "Qwen3.6 cancellation changed its host boundary or retained device state".into(),
        ));
    }

    let joined = generator.admit(prefill)?;
    if joined.native_prefill_tokens != 128
        || generator.qualification_slot(joined.request_id) != Some(1)
    {
        return Err(Qwen36GenerationQualificationError::Mismatch(format!(
            "concurrent T=128 admission selected native={} slot={:?}",
...
fn verify_owner(
    ...
    if defaults.temperature != 1.0 || defaults.top_p != 0.95 || defaults.top_k != 20 {
        return Err(Qwen36GenerationQualificationError::Mismatch(
            "Qwen3.6 generation defaults changed".into(),
        ));
    }
...
fn verify_compact_owner(
    generator: &Qwen36ResidentBatchGenerator,
) -> Result<(), Qwen36GenerationQualificationError> {
    if generator.arena_bytes() != 23_731_989_504
        || generator.resident_weight_bytes() != 19_808_036_096
        || generator.host_stager_bytes() != 8_503_296
        || generator.context_capacity() != 262_144
    {
        return Err(Qwen36GenerationQualificationError::Mismatch(format!(
            "Qwen3.6 compact owner has arena={} weights={} host={} context={}",
            generator.arena_bytes(),
            generator.resident_weight_bytes(),
    ...
    if addresses.len() != 43 || unique.len() != addresses.len() || addresses.contains(&0) {
        return Err(Qwen36GenerationQualificationError::Mismatch(
            "Qwen3.6 compact owner addresses are invalid".into(),
        ));
    }
    Ok(())
}
...
#[cfg(test)]
mod tests {
    use super::{qualify_qwen36_compact_generation, qualify_qwen36_generation};
    use std::path::PathBuf;

    #[test]
    #[ignore = "requires the pinned Qwen3.6 snapshot and an exclusive SM120 device"]
    fn source_frontend_generation_matches_transformers_and_streaming()
    -> Result<(), super::Qwen36GenerationQualificationError> {
        let root = std::env::var_os("TUISKO_QWEN36_SNAPSHOT").ok_or_else(|| {
            super::Qwen36GenerationQualificationError::Mismatch(
                "set TUISKO_QWEN36_SNAPSHOT to the admitted revision".into(),
            )
        })?;
    ...
    -> Result<(), super::Qwen36GenerationQualificationError> {
        ...
        let report = qualify_qwen36_compact_generation(&PathBuf::from(root))?;
        ...
        assert_eq!(report.arena_bytes, 23_731_989_504);
        assert_eq!(report.host_stager_bytes, 8_503_296);
        assert_eq!(report.stable_addresses, 43);
        eprintln!("Qwen3.6 compact generation qualification passed: {report:?}");
        Ok(())
    }
}
