...
use tuisko_qual::{
    ...
    benchmark_qwen36_fp8_attention_qk_prepare, benchmark_qwen36_fp8_paged_gqa,
    ...
    benchmark_qwen38_flash_next_qsa_attention, benchmark_qwen38_flash_next_qsa_prepare,
    ...
    benchmark_text_endpoint, profile_resident_model, profile_resident_prefill, qualify_fp8_lm_head,
};
...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    let suite = arguments
        .next()
        .ok_or("usage: bench-device <attention-qk-prepare|qwen35-attention-qk-prepare|qwen36-attention-qk-prepare|qwen36-fp8-attention-qk-prepare|paged-gqa|qwen35-paged-gqa|qwen36-paged-gqa|qwen36-fp8-paged-gqa|long-context-paged-gqa|attention-output|qwen35-nvfp4-attention-output|qwen36-attention-output|mtp-bf16-fusion|mtp-bf16-qkv|mtp-bf16-qk-prepare|mtp-bf16-paged-gqa|mtp-bf16-attention-output|mtp-bf16-mlp|mtp-layer|qwen35-mtp-layer|qwen36-mtp-layer|mtp-prompt-prime|resident-mtp|qwen35-resident-mtp|qwen35-mtp-generation|generation-mtp-greedy|generation-mtp-sampling|generation-mtp-batch|target-mtp-verify|qwen38-flash-next-hyper-connection|residual-norm|qwen35-residual-norm|qwen35-nvfp4-swiglu|qwen35-nvfp4-down|qwen35-nvfp4-qkv|qwen35-nvfp4-gdn-input|qwen35-gdn-prepare|qwen35-gdn-recurrence|qwen35-nvfp4-gdn-output|qwen35-nvfp4-mlp|qwen35-text-endpoint|qwen35-resident-model|qwen36-residual-norm|qwen36-moe-router|qwen36-moe-experts|qwen36-nvfp4-lm-head|qwen36-text-endpoint|qwen36-resident-model|qwen36-fp8-qkv|qwen36-gdn-input|qwen36-gdn-prepare|qwen36-gdn-recurrence|qwen36-gdn-output|qwen36-gdn-moe-layer|qwen36-full-attention-layer|fp8-qkv|fp8-gdn-input|fp8-lm-head|fp8-swiglu|fp8-down|nvfp4-swiglu|nvfp4-down|nvfp4-mlp|gdn-prepare|gdn-recurrence|gdn-output|qwen38-flash-next-gdn-prepare|qwen38-flash-next-gdn-recurrence|qwen38-flash-next-qsa-prepare|qwen38-flash-next-qsa-attention|dense-fp8-mlp|dense-fp8-gdn-layer|full-attention-layer|qwen35-full-attention-layer|qwen35-gdn-layer|resident-model|resident-prefill|resident-long-context-model|text-endpoint|profile-resident-model|profile-resident-prefill|qualify-fp8-lm-head> [SNAPSHOT] [options]")?;
    #[cfg(feature = "device")]
    ...
    let report = match suite.as_str() {
        ...
        "qwen35-attention-qk-prepare" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen35_attention_qk_prepare(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "qwen36-attention-qk-prepare" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen36_attention_qk_prepare(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "qwen36-fp8-attention-qk-prepare" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (
                benchmark_qwen36_fp8_attention_qk_prepare(options)?,
                json_path,
            )
        }
        #[cfg(feature = "device")]
        ...
        "qwen35-paged-gqa" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen35_paged_gqa(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "qwen36-paged-gqa" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen36_paged_gqa(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "qwen36-fp8-paged-gqa" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen36_fp8_paged_gqa(options)?, json_path)
        }
        #[cfg(feature = "device")]
        ...
        "mtp-bf16-fusion" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_mtp_bf16_fusion(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "mtp-bf16-qkv" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_mtp_bf16_qkv(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "mtp-bf16-qk-prepare" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_mtp_bf16_qk_prepare(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "mtp-bf16-paged-gqa" => {
            let (options, json_path) =
        ...
        "mtp-bf16-attention-output" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_mtp_bf16_attention_output(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "mtp-bf16-mlp" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_mtp_bf16_mlp(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "mtp-layer" => {
            let snapshot = arguments
                .next()
                .ok_or("mtp-layer requires the admitted snapshot path")?;
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::long_graph())?;
            (
                benchmark_mtp_layer(&PathBuf::from(snapshot), options)?,
                json_path,
            )
        }
        #[cfg(feature = "device")]
        "qwen35-mtp-layer" => {
            let snapshot = arguments
                .next()
                .ok_or("qwen35-mtp-layer requires the admitted snapshot path")?;
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::long_graph())?;
            (
        ...
        "mtp-prompt-prime" => {
            let snapshot = arguments
                .next()
                .ok_or("mtp-prompt-prime requires the admitted snapshot path")?;
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::long_graph())?;
            (
                benchmark_mtp_prompt_prime(&PathBuf::from(snapshot), options)?,
                json_path,
            )
        }
        #[cfg(feature = "device")]
        "resident-mtp" => {
            let snapshot = arguments
                .next()
                .ok_or("resident-mtp requires the admitted snapshot path")?;
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::long_graph())?;
            (
                benchmark_resident_mtp(&PathBuf::from(snapshot), options)?,
                json_path,
            )
        }
        #[cfg(feature = "device")]
        "qwen35-resident-mtp" => {
            let snapshot = arguments
                .next()
                .ok_or("qwen35-resident-mtp requires the admitted snapshot path")?;
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::long_graph())?;
            (
                benchmark_qwen35_resident_mtp(&PathBuf::from(snapshot), options)?,
                json_path,
            )
        }
        #[cfg(feature = "device")]
        "qwen35-mtp-generation" => {
            let snapshot = arguments
                .next()
                .ok_or("qwen35-mtp-generation requires the admitted snapshot path")?;
            let (options, json_path) =
        ...
        "generation-mtp-greedy" => {
            let snapshot = arguments
                .next()
                .ok_or("generation-mtp-greedy requires the admitted snapshot path")?;
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::resident_model())?;
            (
                benchmark_resident_mtp_generation(&PathBuf::from(snapshot), options)?,
                json_path,
            )
        }
        #[cfg(feature = "device")]
        "generation-mtp-sampling" => {
            let snapshot = arguments
                .next()
                .ok_or("generation-mtp-sampling requires the admitted snapshot path")?;
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::resident_model())?;
            (
                benchmark_resident_mtp_sampling(&PathBuf::from(snapshot), options)?,
                json_path,
            )
        }
        #[cfg(feature = "device")]
        "generation-mtp-batch" => {
            let snapshot = arguments
                .next()
                .ok_or("generation-mtp-batch requires the admitted snapshot path")?;
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::resident_model())?;
            (
                benchmark_resident_mtp_batch_generation(&PathBuf::from(snapshot), options)?,
                json_path,
            )
        }
        #[cfg(feature = "device")]
        ...
        "qwen35-nvfp4-attention-output" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen35_nvfp4_attention_output(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "qwen36-attention-output" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen36_attention_output(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "qwen38-flash-next-hyper-connection" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (
                benchmark_qwen38_flash_next_hyper_connection(options)?,
                json_path,
            )
        }
        "residual-norm" => {
        ...
        "qwen35-nvfp4-qkv" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen35_nvfp4_qkv(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "qwen36-moe-router" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen36_moe_router(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "qwen36-moe-experts" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen36_moe_experts(options)?, json_path)
        }
        #[cfg(feature = "device")]
        ...
        "qwen36-gdn-input" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen36_gdn_input(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "qwen36-fp8-qkv" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen36_fp8_qkv(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "qwen36-gdn-prepare" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen36_gdn_prepare(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "qwen36-gdn-recurrence" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen36_gdn_recurrence(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "qwen36-gdn-output" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen36_gdn_output(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "qwen35-nvfp4-gdn-input" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen35_nvfp4_gdn_input(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "qwen35-gdn-prepare" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen35_gdn_prepare(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "qwen35-gdn-recurrence" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen35_gdn_recurrence(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "qwen35-nvfp4-gdn-output" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen35_nvfp4_gdn_output(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "qwen35-nvfp4-mlp" => {
            let snapshot = arguments
        ...
        "qwen38-flash-next-gdn-prepare" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen38_flash_next_gdn_prepare(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "qwen38-flash-next-gdn-recurrence" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (
                benchmark_qwen38_flash_next_gdn_recurrence(options)?,
                json_path,
            )
        }
        #[cfg(feature = "device")]
        "qwen38-flash-next-qsa-prepare" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen38_flash_next_qsa_prepare(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "qwen38-flash-next-qsa-attention" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (
                benchmark_qwen38_flash_next_qsa_attention(options)?,
                json_path,
            )
        }
        #[cfg(feature = "device")]
        ...
        "qwen35-full-attention-layer" => {
            let snapshot = arguments
                .next()
                .ok_or("qwen35-full-attention-layer requires the admitted snapshot path")?;
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::long_graph())?;
            (
                benchmark_qwen35_full_attention_layer(&PathBuf::from(snapshot), options)?,
                json_path,
            )
        }
        #[cfg(feature = "device")]
        "qwen35-gdn-layer" => {
            let snapshot = arguments
                .next()
                .ok_or("qwen35-gdn-layer requires the admitted snapshot path")?;
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::long_graph())?;
            (
        ...
        "qwen36-gdn-moe-layer" => {
            let snapshot = arguments
                .next()
                .ok_or("qwen36-gdn-moe-layer requires the admitted snapshot path")?;
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::long_graph())?;
            (
                benchmark_qwen36_gdn_moe_layer(&PathBuf::from(snapshot), options)?,
                json_path,
            )
        }
        #[cfg(feature = "device")]
        "qwen36-full-attention-layer" => {
            let snapshot = arguments
                .next()
                .ok_or("qwen36-full-attention-layer requires the admitted snapshot path")?;
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::long_graph())?;
            (
                benchmark_qwen36_full_attention_layer(&PathBuf::from(snapshot), options)?,
                json_path,
            )
        }
        #[cfg(feature = "device")]
        "qwen35-text-endpoint" => {
            let snapshot = arguments
                .next()
                .ok_or("qwen35-text-endpoint requires the admitted snapshot path")?;
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::long_graph())?;
            (
                benchmark_qwen35_text_endpoint(&PathBuf::from(snapshot), options)?,
                json_path,
            )
        }
        #[cfg(feature = "device")]
        "qwen36-text-endpoint" => {
            let snapshot = arguments
                .next()
                .ok_or("qwen36-text-endpoint requires the admitted snapshot path")?;
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::long_graph())?;
            (
                benchmark_qwen36_text_endpoint(&PathBuf::from(snapshot), options)?,
                json_path,
            )
        }
        #[cfg(feature = "device")]
        "qwen35-resident-model" => {
            let snapshot = arguments
                .next()
                .ok_or("qwen35-resident-model requires the admitted snapshot path")?;
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::resident_model())?;
            (
                benchmark_qwen35_resident_model(&PathBuf::from(snapshot), options)?,
                json_path,
            )
        }
        #[cfg(feature = "device")]
        "qwen36-resident-model" => {
            let snapshot = arguments
                .next()
                .ok_or("qwen36-resident-model requires the admitted snapshot path")?;
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::resident_model())?;
            (
                benchmark_qwen36_resident_model(&PathBuf::from(snapshot), options)?,
                json_path,
            )
        }
        #[cfg(feature = "device")]
        ...
        "resident-prefill" => {
            let snapshot = arguments
                .next()
                .ok_or("resident-prefill requires the admitted snapshot path")?;
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::resident_model())?;
            (
                benchmark_resident_prefill(&PathBuf::from(snapshot), options)?,
                json_path,
            )
        }
        #[cfg(feature = "device")]
