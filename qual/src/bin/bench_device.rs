...
use tuisko_qual::{
    ...
    benchmark_qwen38_flash_next_qsa_selection, benchmark_resident_long_context_model,
    ...
    let report = match suite.as_str() {
        ...
        "qwen38-flash-next-qsa-selection" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (
                benchmark_qwen38_flash_next_qsa_selection(options)?,
                json_path,
            )
        }
        #[cfg(feature = "device")]
        "qwen38-flash-next-moe-router" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen38_flash_next_moe_router(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "qwen38-flash-next-moe-experts" => {
            let (options, json_path) =
                parse_options(arguments, DeviceBenchmarkOptions::short_graph())?;
            (benchmark_qwen38_flash_next_moe_experts(options)?, json_path)
        }
        #[cfg(feature = "device")]
        "gdn-recurrence" => {
            let (options, json_path) =
