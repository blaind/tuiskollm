...
use tuisko_qual::{
    ...
    benchmark_resident_mtp_generation, benchmark_resident_prefill, benchmark_target_mtp_verify,
...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    let suite = arguments
        .next()
        .ok_or("usage: bench-device <attention-qk-prepare|qwen35-attention-qk-prepare|paged-gqa|qwen35-paged-gqa|long-context-paged-gqa|attention-output|qwen35-nvfp4-attention-output|mtp-bf16-fusion|mtp-bf16-qkv|mtp-bf16-qk-prepare|mtp-bf16-paged-gqa|mtp-bf16-attention-output|mtp-bf16-mlp|mtp-layer|mtp-prompt-prime|resident-mtp|generation-mtp-greedy|target-mtp-verify|residual-norm|qwen35-residual-norm|qwen35-nvfp4-swiglu|qwen35-nvfp4-down|qwen35-nvfp4-qkv|qwen35-nvfp4-mlp|fp8-qkv|fp8-gdn-input|fp8-lm-head|fp8-swiglu|fp8-down|nvfp4-swiglu|nvfp4-down|nvfp4-mlp|gdn-prepare|gdn-recurrence|gdn-output|dense-fp8-mlp|dense-fp8-gdn-layer|full-attention-layer|qwen35-full-attention-layer|resident-model|resident-prefill|resident-long-context-model|text-endpoint|profile-resident-model|qualify-fp8-lm-head> [SNAPSHOT] [options]")?;
    #[cfg(feature = "device")]
