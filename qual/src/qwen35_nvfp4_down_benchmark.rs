...
use crate::qwen35_nvfp4_down::{
    ...
    MAX_BATCH, OUTPUT_ROWS, Regions, WEIGHT_SCALE_DIVISOR, layout, make_fixture,
};
    ...
    fn new(repeated_operations: u64) -> Result<Self, DeviceBenchmarkError> {
        ...
        let fixture = make_fixture().map_err(|error| {
            DeviceBenchmarkError::Precondition(format!(
                "Qwen3.5 NVFP4 down fixture construction failed: {error}"
            ))
        })?;
    ...
    fn cases(&self, repeated_operations: u64) -> Vec<ExactDeviceCase<'_>> {
        self.routes
            ...
            .map(|route| {
                ...
                ExactDeviceCase::new(
                    "qwen35_9b/nvfp4_down/production",
                    shape,
                    workload,
...
fn launch(
    ...
    // SAFETY: each address names its aligned maximum-row arena region.
    unsafe {
        if rows <= MAX_BATCH {
            op.launch(
                stream,
...
mod tests {
    ...
    #[test]
    fn accounting_covers_every_qwen35_decode_and_prefill_byte() {
        let (layout, regions) = layout().unwrap();
        let weights = OUTPUT_ROWS * (CODE_BYTES_PER_ROW + GROUPS_PER_ROW);
        let t1024 = weights
            + super::super::qwen35_nvfp4_down::MAX_ROWS
                * (INPUT_COLUMNS * size_of::<u16>()
                    + OUTPUT_ROWS * size_of::<u16>()
                    + 2 * (CODE_BYTES_PER_ROW + GROUPS_PER_ROW));

        assert_eq!(logical_bytes(1_024), t1024);
