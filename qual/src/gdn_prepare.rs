...
pub fn qualify_gdn_prepare() -> Result<GdnPrepareQualification, GdnPrepareQualificationError> {
    preflight().map_err(|error| GdnPrepareQualificationError::Mismatch(error.to_string()))?;
    ...
    require_current_process_exclusive()
        .map_err(|error| GdnPrepareQualificationError::Mismatch(error.to_string()))?;
...
fn verify_oracle(
    ...
    report.control_values += rows * 2 * Qwen38_27B::GDN_CONTROL_ROWS;
    report.convolution_values += rows * Qwen38_27B::GDN_QKV_ROWS;
...
fn inactive_values(rows: usize) -> usize {
    (MAX_ROWS - rows) * (2 * Qwen38_27B::GDN_CONTROL_ROWS + Qwen38_27B::GDN_QKV_ROWS)
}
...
fn verify_replay(
    ...
    {
        return Err(GdnPrepareQualificationError::Mismatch(format!(
            "rows={rows} graph control value {index} differs from eager"
        )));
    }
    ...
    {
        return Err(GdnPrepareQualificationError::Mismatch(format!(
            "rows={rows} graph convolution value {index} differs from eager"
        )));
    }
    if let Some(index) = replay
        .history
    ...
    {
        return Err(GdnPrepareQualificationError::Mismatch(format!(
            "rows={rows} graph history value {index} differs from eager"
        )));
    }
    verify_inactive(rows, replay)?;

    report.graph_replay_values +=
        rows * (2 * Qwen38_27B::GDN_CONTROL_ROWS + Qwen38_27B::GDN_QKV_ROWS) + replay.history.len();
    report.inactive_values += inactive_values(rows);
    report.immutable_values += immutable_values(fixture);
