...
/// Qualifies eager and captured Qwen3.6 router execution at every exact route.
...
pub(crate) fn layout() -> GpuResult<(ArenaLayout, Regions)> {
    ...
    let logits = layout.reserve(MAX_ROWS * EXPERTS, ALIGNMENT)?;
    let expert_indices = layout.reserve(MAX_ROWS * TOP_K, ALIGNMENT)?;
    let expert_weights = layout.reserve(MAX_ROWS * TOP_K, ALIGNMENT)?;
...
pub(crate) fn make_fixture() -> Fixture {
    ...
    let weights = (0..EXPERTS * HIDDEN)
        ...
        .collect::<Vec<_>>();
    let mut expected_logits = vec![0; MAX_ROWS * EXPERTS];
    let mut expected_indices = vec![0; MAX_ROWS * TOP_K];
    let mut expected_weights = vec![0; MAX_ROWS * TOP_K];
...
fn verify_eager(
    ...
    if observed.logits[..logits] != fixture.expected_logits[..logits] {
        ...
        return Err(Qwen36MoeRouterQualificationError::Mismatch(format!(
            "rows={rows} logit {index}: device={:#06x}, oracle={:#06x}",
            observed.logits[index], fixture.expected_logits[index]
        )));
    }

    let selected = rows * TOP_K;
    if observed.indices[..selected] != fixture.expected_indices[..selected] {
        let index = observed.indices[..selected]
            ...
            .expect("different slices contain one mismatch");
        return Err(Qwen36MoeRouterQualificationError::Mismatch(format!(
            "rows={rows} selected expert {index}: device={}, oracle={}",
    ...
    for index in 0..selected {
        ...
        if absolute_error > 0.001 {
            return Err(Qwen36MoeRouterQualificationError::Mismatch(format!(
                "rows={rows} routing weight {index}: device={actual}, oracle={expected}"
            )));
        }
    }
    ...
    report.inactive_values += (MAX_ROWS - rows) * (EXPERTS + 2 * TOP_K);
...
mod tests {
    ...
    -> Result<(), Qwen36MoeRouterQualificationError> {
        ...
        assert_eq!(report.immutable_input_values, 786_432);
        assert_eq!(report.arena_bytes, 1_642_496);
        assert_eq!(report.weight_bytes, 1_048_576);
        assert_eq!(report.workspace_bytes, 593_920);
