...
/// Qualifies eager and captured Qwen3.6 GDN outputs at every exact route.
...
fn verify_eager(
    ...
    for token in 0..rows {
        for row in 0..OUTPUT_ROWS {
            ...
            if absolute_error > tolerance {
                return Err(Qwen36GdnOutputQualificationError::Mismatch(format!(
                    "rows={rows} token={token}, row={row}: device={actual}, oracle={expected}, tolerance={tolerance}"
                )));
            }
        }
    }
    ...
    report.inactive_values += (MAX_ROWS - rows) * (INPUT_COLUMNS + OUTPUT_ROWS);
...
fn verify_replay(
    ...
    report.graph_replay_values += rows * (INPUT_COLUMNS + OUTPUT_ROWS);
    report.inactive_values += (MAX_ROWS - rows) * (INPUT_COLUMNS + OUTPUT_ROWS);
...
mod tests {
    ...
    -> Result<(), Qwen36GdnOutputQualificationError> {
        ...
        assert_eq!(report.immutable_input_values, 8_912_896);
        assert_eq!(report.arena_bytes, 10_485_760);
        assert_eq!(report.weight_bytes, 8_388_608);
        assert_eq!(report.workspace_bytes, 2_097_152);
