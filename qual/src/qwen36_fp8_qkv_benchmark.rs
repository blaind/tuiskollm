//! Paired timings for every exact Qwen3.6 full-attention QKV route.
...
impl Session {
    fn new(repeated_operations: u64) -> Result<Self, DeviceBenchmarkError> {
        ...
        let fixture = make_fixture().map_err(|error| {
            DeviceBenchmarkError::Precondition(format!("building Qwen3.6 QKV fixture: {error}"))
        })?;
...
pub fn benchmark_qwen36_fp8_qkv(
    ...
    memory.register_owned(
        "qwen36_35b_a3b/full_attention/qkv_weights",
        BenchmarkMemoryKind::Weights,
        weight_bytes,
        "source E4M3 Q/gate, K, and V planes",
    )?;
    memory.register_owned(
