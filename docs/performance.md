...
| `cargo run -p xtask -- bench-resident-mtp SNAPSHOT` | Directly measure every resident long-context MTP seeded-draft and residual-continuation B=1..8 graph, including pinned input upload, the respective target/prior-residual handoff, shared target LM head, and the production cache regime | terminal or `--json PATH` |
