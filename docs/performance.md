...
| `cargo run -p xtask -- qualify-resident-model SNAPSHOT` | Check all 64 source routes, final source-backed formulas, noncontiguous physical slots and isolated reset, persistent state/cache, exact-B whole-model graphs, stable addresses, and owner allocation | terminal |
...
| `cargo run -p xtask -- qualify-resident-batch-generation SNAPSHOT` | Compare compact mixed-length scheduling with sequential requests, including noncontiguous survivor replay, middle-slot recycling, stable ownership, and zero post-warmup device allocation | terminal |
